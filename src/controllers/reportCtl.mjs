...
export async function reportWithLocation(req, res) {
  try {
    const { latitude, longitude, hazard_level_25yr = null } = matchedData(req);
    const { user_id } = whoIsUser(req);

    const report = await reportServ.logReportWithCoordinates({ latitude, longitude, hazard_level_25yr, user_id });
